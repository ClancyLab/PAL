...
RANGE = 1000
sim_methods = ["misokg"]
submitted_sims = ["diag_B_as_Ks", "diag_B_as_L", "scaled"]
...
for folder in ["data_dumps"]:
    ...
    costs = {k: [] for k in submitted_sims}
    ...
    for fptr in os.listdir(folder):
        for sim in sim_methods:
            if fptr.endswith("_%s.log" % sim):
                ...
                if cost is not None:
                    k = int(fptr.split("_")[0])
                    l_sim = submitted_sims[int(np.floor(k / RANGE))]
                    costs[l_sim].append(cost)
